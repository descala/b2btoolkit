...
def main(fitxer, extension, path, pathcva)
  ...
  1.upto(oo.sheets.length - 1) do |sheet|
    ...
    if (oo.sheets[sheet] == "CVA") then
      ...
      2.upto(oo.last_row) do |line|
        ...
        id = oo.cell(line,'B')
        item = oo.cell(line,'C')
		itemcefact = oo.cell(line,'D')
        scope = oo.cell(line,'E')
		scopecefact = oo.cell(line,'F')
        values = oo.cell(line,'G')
        message = oo.cell(line,'H')
        severity = oo.cell(line,'I')
        ...
        if transaction != last_transaction then
		  ...
		  cvacefact = ValueListConstraints.new(transaction)
          list_of_cvacefact << cvacefact if cvacefact
          valuelist.each { |vl| cvacefact.add_valuelist(vl) }
        end
      
        ctx = Context.new(id, item, scope, values, severity, message)    
        cva.add_context(ctx) if ctx

        ctxcefact = Context.new(id, itemcefact, scopecefact, values, severity, message)    
        cvacefact.add_context(ctxcefact) if ctxcefact
      ...
      0.upto(list_of_cva.length() - 1) do |cvafile|
		...
		puts "Creating UBL CVA file for "+extension+list_of_cva[cvafile].id+"..."
        file = File.new(extension+"Codes"+list_of_cva[cvafile].id+"-UBL.cva", "w")
        file.puts list_of_cva[cvafile].to_xml
	  end
      0.upto(list_of_cvacefact.length() - 1) do |cvafile|
        
		puts "Creating CEFACT CVA file for "+extension+list_of_cvacefact[cvafile].id+"..."
		filecefact = File.new(extension+"Codes"+list_of_cvacefact[cvafile].id+"-CEFACT.cva","w")
        filecefact.puts list_of_cvacefact[cvafile].to_xml
		end
	 end
  end
end
