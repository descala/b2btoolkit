...
def main(fitxer,path,extension)
 ...
 # oo = Excel.new(fitxer)
  ...
  artifact_sheet="artifacts"
   ...
   2.upto(oo.last_row) do |line|
	  ...
	  codelist	  = oo.cell(line,'D')
	  namespace   = oo.cell(line,'E')
	  namespacecac   = oo.cell(line,'F')
	  namespacecbc   = oo.cell(line,'G')
...
class Schematron
  ...
  def to_xml
    ...
    xml.schema("xmlns" => "http://purl.oclc.org/dsdl/schematron", 
                ...
                "xmlns:""#{@binding}" => "#{@namespace}",
                :queryBinding => "xslt2") {
                  xml.title(extension.upcase+" #{@profile} #{@transaction} bound to "+binding.upcase)
				  
				  if @binding=="CEFACT" then
                  xml.ns(:prefix => "udt", :uri => "#{@namespacecbc}")
                  xml.ns(:prefix => "ram", :uri => "#{@namespacecac}")
                  xml.ns(:prefix => "rsm", :uri => "#{@namespace}")
